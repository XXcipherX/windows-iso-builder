...
- The full installation test requires x64 KVM, image index 1, and at least 25 GiB free in the selected temporary work area. It derives a CI answer-file overlay from the ISO, sends boot keys only during a bounded window after OVMF starts the installation DVD, and requires the installed disk to reach Windows Boot Manager within 20 minutes. It automates only the ephemeral VM and never uploads the virtual disk.
